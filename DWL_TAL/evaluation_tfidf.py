...
for queryID in parse_queries.keys():
    ...
    vecteur_requete = projet_tfidf.traitement_requete(parse_queries[queryID]['text'])
    vecteur_init = projet_tfidf.scores_simi(vecteur_requete, n = max_qrels)
    print("--------------------------------------")
    for doc_id,score in vecteur_init[:5]:
        titre = doc_film[doc_id]["title"]
        print(f"{titre} (Score de similarité : {score})")

    vecteur_augmente = pseudo_relevance_feedback(projet_tfidf.tf_idf, vecteur_requete, vecteur_init, qrels_ui, alpha = 1.0, beta = 0.6, k = 15)
    print("----------------------PRF-------------------------")
    query_ui = projet_tfidf.scores_simi(vecteur_augmente, n = max_qrels)
